...
`spiking-aeon` (Package 26) is a standalone GenesisAeon Entropy Atlas
package with no GenesisAeon-ecosystem runtime dependency — its UTAC/CREP
logic is self-contained. If that ever changes, pin any
`GenesisAeon/*` dependency with `>=` lower bounds matching the minimum
version that provides the API this package relies on. Do not pin exact
versions (`==`) for ecosystem dependencies.
